...
export class CongratulationCommand extends Command {
  ...
  constructor(
    ...
    super(client);
	this.telegramApiService = new TelegramApiService(this.configService, this.managersRepository, this.telegramApiRepository)
  }

	async handle(): Promise<void> {
		this.client.action('cb_congratulation', async (ctx) => {
			const managerName = this.telegramApiService.managerName;
			this.logger.log(ctx.from.username + ` запуск колбэка поздравления ` + managerName)
