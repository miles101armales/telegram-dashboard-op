...
export class MySalesCommand extends Command {
	...
	async handle(): Promise<void> {
      ...
      if(authStatus.authorization) {
        this.handled(ctx);
		this.client.action('personal_goal_edit', async ctx => {this.personalGoalEdit(ctx)});
      } else {
        ctx.reply('Авторизация не пройдена, /auth')
      }
		})
		this.client.hears('Моя команда', async ctx => {return ctx.replyWithHTML('В разработке')})
	}

	async handled(ctx: MyContext) {
		const manager = await this.telegramApiRepository.findOne({where: {chat_id: ctx.chat.id.toString()}})
		const statistics = await this.managersRepository.findOne({where:{name:manager.manager}})
		...
		if(statistics){
			return ctx.replyWithHTML(
				...
				{ reply_markup: {
					inline_keyboard: [[{ text: 'Изменить личный план', callback_data: 'personal_goal_edit'}]]
				}}
			)
		} else {
	...
	async personalGoalEdit(ctx) {
		...
		this.client.hears(/^\d{7}$/, async ctx => {
			const status = await this.managersRepository.update({name: manager.manager}, {personal_monthly_goal: ctx.msg.text})
			if(status) {
